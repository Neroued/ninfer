...
std::string existing_read_path(const std::string& path) {
    ...
    const std::filesystem::path source_relative =
        (std::filesystem::path(NINFER_SOURCE_DIR) / std::filesystem::path(path)).lexically_normal();
...
std::string format_table(const BenchEnvironment& env, const std::vector<TestResult>& results) {
    std::ostringstream out;
    out << "ninfer_bench throughput report\n";
...
#ifdef NINFER_SOURCE_DIR
    return run_command_capture("git -C " + shell_quote(NINFER_SOURCE_DIR) + " rev-parse HEAD");
#else
    return {};
#endif
}
...
bool current_git_worktree_dirty() {
    ...
    const std::string out =
        run_command_capture("git -C " + shell_quote(NINFER_SOURCE_DIR) + " status --porcelain");
    return !out.empty();
#else
    return false;
#endif

...
KvCacheStorage parse_kv_cache(std::string_view text) {
    ...
    throw std::invalid_argument("--kv-dtype must be bf16, int8, fp8, nvfp4, k8v4, or rk2v4-e8");
}
...
std::string usage_text(std::string_view program) {
    ...
    out << "Usage: " << program << " --weights <artifact.ninfer> [options]\n\n"
        ...
        << "  --kv-dtype <bf16|int8|fp8|nvfp4|k8v4|rk2v4-e8>  KV cache storage (default: bf16)\n"

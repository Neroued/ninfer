...
DBuf make_logits(int cols) {
    ...
    for (int c = 0; c < cols; ++c) {
        const int hot = (17 + c * 7919) % kTokenDomain;
...
double payload_bytes(int cols, bool counts) {
    const double per_col = static_cast<double>(kTokenDomain) * 2.0 +
                           (counts ? static_cast<double>(kTokenDomain) * 4.0 : 0.0);
...
void run_argmax(DBuf& logits, int cols) {
    ...
    const Result r = bench_loop(
        [&](cudaStream_t s) { kernels::argmax(tlogits, tout, kTokenDomain, s); },
...
void run_sample(DBuf& logits, DBuf& cfg, int cols) {
    ...
    const Result r = bench_loop(
        [&](cudaStream_t s) {
            kernels::sample(tlogits, tout, kTokenDomain, cfg_ptr, pos_ptr,
...
int main(int argc, char** argv) {
    ...
    std::printf("payload: physical_rows=%d token_domain=%d logits=%.3f MiB/col "
                "counts=%.3f MiB/col sampling=%.3f MiB/col\n",
                kVocab, kTokenDomain, static_cast<double>(kTokenDomain * 2) / 1048576.0,
                static_cast<double>(kTokenDomain * 4) / 1048576.0,
                payload_bytes(1, true) / 1048576.0);

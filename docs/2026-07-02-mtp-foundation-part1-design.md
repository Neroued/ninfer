...
acceptance/rejection sampler。本文也不引入新的量化版本号；所有约定都在现有
`q5090_w4g64_mixed_v3` 和现有 `W8G128_F16S` qtype 语义内原地收敛。
...
当前 Python tensor plan 中的 MTP layout 是 15 个 standalone specs：
...
其中所有 MTP linears 是 `W8G128_F16S` `ROW_SPLIT` `group_size=128`。这部分 qtype 与
packing 不需要重新设计，但 standalone layout 不利于 runtime：

- raw `self_attn.q_proj.weight [12288,5120]` 同时包含 query 与 gate；
- gate/up 是两个独立 W8 blocks，无法共享 input activation；
- attention input 需要四次 W8 linear launch；
- decode 路径需要额外 split/deinterleave 或不理想的输出读取。
...
pattern, except MTP uses a single W8 block rather than separate Q4 and Q5 blocks.
...
- W8 row-split payload layout: one signed int8 code per K element, no high plane, FP16 scale per
  `(row, K128 group)`;
- `group_size=128`, `scale_dtype=FP16`, `high_plane_bytes=0`;
...
1. `docs/q5090_packed_file_format_v3.md`
   ...
   - Keep `W8G128_F16S` numeric policy unchanged; the v3 spec owns only the new MTP block/segment/fusion
     assignment.

No quantization version bump is introduced by this layout change. It changes block grouping and
source transforms, not numeric qtype semantics.
...
- every W8 MTP block has `group_size=128`, `scale_dtype=FP16`, `high_plane_bytes=0`;
- all MTP norms remain BF16 contiguous.

Packing tests for W8 do not need a new qtype test if existing W8 pack/unpack tests already cover
round-trip and dequant value preservation. Converter-level structural tests must change because the
expected MTP manifest changes.
...
1. `docs/q5090_packed_file_format_v3.md` describes the new MTP_DRAFT fused layout without a version
   bump; no q5090 v1/v2 document is edited for this change.
2. Python converter emits MTP_DRAFT with 12 blocks and 16 segments in the layout above.
...
4. Python verifier checks fused MTP block-level `source_kind == OTHER`, MTP segment row ranges, and MTP
   fusion ids `ATTN_IN=1` / `MLP_GATEUP=3`.

...
- `gdn_in_ab_gated()` 支持 `x [5120,T] -> g/beta [48,T]`。
...
| `gdn_in_ab_gated` | 已有 `[5120,T] -> g,beta [48,T]` unified path | 在 target verify path 直接调用 T=1..6 | 输出很小，已有 prefill test/bench 基础 |
...
| GDN A/B/g/beta | `gdn_in_ab_gated()` T=1 | `gdn_in_ab_gated()` T>1 | performance only |

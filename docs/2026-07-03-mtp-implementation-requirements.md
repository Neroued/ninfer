...
| R-L0-3 | `GdnState` 扩展为 `k+1` 槽位布局（slot 0 = committed；conv `[10240,3,S]`、ssm `[128,128,48,S]`）；`reset` 只清 slot 0；现有单槽路径改为访问 slot 0 | C3；state-management §4.1 |
| R-L0-4 | `StepState`/device 标量扩展：`logits [248320,k+1]`、`target_tokens/drafts/sampled_out/verify_ids/shifted_ids/positions` 缓冲、`L/a/ar_pos/num_sampled` 标量、统计计数器、verify hidden `[5120,k+1]`、chunk hidden `[5120,prefill_chunk]`、AR hidden | C5；state-management §6 |
...
| R-L1-3 | GQA append 原语：先复用 `gqa_attention_prefill(cache_offset)` 做正确性路径；新增 **device 标量窗口基址**变体（graph 化前置）；small-T 调优按 Part 2 §8.2 | C5；state-management §5.3 |
| R-L1-4 | GDN 快照算子对：`causal_conv1d_sequence_snapshot`、`gated_delta_rule_recurrent_snapshot`（契约见 state-management §5.1/§5.2，**取代 Part 2 §9.2 单终态契约**）；`gdn_commit`（slot[a]→slot[0]，device 标量驱动） | C3 |
...
| ID | 需求 | 说明 |
|---|---|---|
| R-RT-1 | round 循环：verify → accept/commit → propose → host 回读；容量守卫 `L + 2k ≤ max_ctx`，否则回退 T=1 decode；round 内禁止 `prefill()` | round-algorithm §2/§3 |
| R-RT-2 | `EngineOptions` 增加 `mtp_draft_tokens k`（0 = 关闭；1..5），load 时定死缓冲；CLI 透传（`--mtp-draft-tokens`，与 ref model 参数同名） | 总领 §7 |
...
| R-RT-5 | strict-sequential 调试模式（verify 改为逐 token T=1 复放，无快照）：CLI 开关；用于 E2E 验收 | 总领 §8.2 |
...
| ID | 需求 |
|---|---|
| R-TOOLS-1 | ref model 维持算法 oracle 地位；补充（可选）round 级状态 dump 对拍入口（GDN slot 0 / MTP KV 与 C++ 侧对齐，state-management §7） |
...
1. **数值正确性（白名单 1）**：W8G32 linear（真实 K 对齐 + 小合成形状，T=1/2/5/17）；
   ...
   GDN 快照算子——含关键性质「`states[...,j]` 与同 kernel 连续 j+1 次 T=1 调用
   逐 bit 相等」（state-management §5.5）；`gdn_commit` 选槽正确性。
...
3. **CLI/报告契约（白名单 3）**：bench schema 新字段（`qus_bench_support` 契约
   测试延伸）。
4. **端到端行为（白名单 4，canonical fixtures）**：strict-sequential 模式下
   MTP-on 输出 == MTP-off greedy 输出（固定 prompt 集）；batched 模式对
   strict-sequential 的 near-tie 分叉规则（总领 §8.2）。

...
| **M3 target verify 路径与状态底座** | R-L0-1/3/4、R-L1-4/7、R-L2-2/3、R-L2-5(verify 项) | KV rewind、GDN 快照槽与快照算子、Verify 相位（T=k+1 全列 hidden/logits/argmax）、prefill 全列 hidden | 快照算子与 commit 语义通过 focused tests；sanitizer 干净；记录 verify 成本 checkpoint |

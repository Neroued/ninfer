...
- 引擎支持 `--mtp-draft-tokens k`（k ∈ [1,5]，load 时固定）；默认值由 M5 的 bench
  数据选定。关闭时行为与现状完全一致。
- greedy 语义下：MTP batched 路径提交的 token 序列对自身 verify 结果自洽；
  不以 MTP-on/off 或跨实现输出一致作为验收目标。
...
| **M2 MTP head C++ 前向** | R-L2-1/4、R-L0-2、R-L1-2、R-L2-5(MTP 项) | MTP 绑定 + `mtp_forward`（shifted pass / AR step）+ MTP KV | MTP 权重绑定、pack/split、cache/workspace 契约通过 focused tests、real-file load、sanitizer 和代码审查 |
| **M3 target verify 路径与状态底座** | R-L0-1/3/4、R-L1-4/7、R-L2-2/3、R-L2-5(verify 项) | KV rewind、GDN 快照槽与 selector 快照算子、Verify 相位（T=k+1 全列 hidden/logits/argmax）、prefill 全列 hidden | 快照算子 selected-slot 语义通过 focused tests；sanitizer 干净；记录 verify 成本 checkpoint |
| **M4 引擎 round 循环与验收** | R-RT-1..4/7、R-L1-5/6、R-TOOLS-2(schema) | 完整 round 状态机、prefill 期 MTP KV 填充、统计与 bench schema | canonical prompts 覆盖 batched MTP token 数、round/fallback/acceptance 统计、容量 fallback、stop/max_new 行为；k=5 内存预算成立；sanitizer E2E 干净 |
| **M5 性能化** | R-RT-6、R-L1-3(device 标量/small-T 调优)、R-TOOLS-2(报告)、Part 2 Class C/Phase 2-3 候选 | round 级 CUDA graph、small-T 调优 kernel、k 默认值、bench/nsys/ncu 证据报告 | 端到端 tok/s 相对 MTP-off 的实测加速入档；总领 §7.2 模型与实测对账；graph 路径自洽并通过 MTP 行为 gates |
...
- **做**：`ModelConfig`/绑定结构（`MtpW`）；第二个 `KVCache` 实例；
  ...
  （shifted pass 与 AR step 都用统一 `gqa_attention(..., positions, ...)` 作 append）；
  workspace/cache 公式的 MTP 项。
- **不做**：engine round 逻辑、target 侧任何改动、draft/verify 状态机。
- 可独立验证：MTP 绑定、pack/split、KV 写入边界和 workspace/cache 预算不需要 round
  循环存在。

### M3 — target verify 路径与状态底座（Track B，仅依赖 M0）

- **做**：`KVCache` 回退 API；`GdnState` 扩展为每层 conv/ssm 各一个大 tensor、
  逻辑 k+1 槽位，并新增 `gdn_initial_slot` selector；
  快照算子对（`causal_conv1d_sequence_snapshot`、
  `gated_delta_rule_recurrent_snapshot`）读取 selected 初始槽并顺序写槽；T=1-only fused 算子的
...
- **做**：round 状态机（verify → accept/commit → propose → host 回读）与容量守卫；
  ...
  （消除 decode_step/record 的 advance 不对称）；统计计数器 + bench schema 的
  MTP 段；canonical fixture E2E 测试与 sanitizer E2E。
...
- **做**：attention append 的 device 标量窗口基址变体与整轮 CUDA graph 捕获；
  ...
  小 kernel 进一步调优；k 默认值由 bench corpus 数据选定；
...
- **verify（M3）先于 round（M4）**：GDN 快照语义是全项目风险最高的新机制，必须在
  没有 round 状态机干扰的情况下单独钉死；M4 才能把失败归因收敛到胶水逻辑。
- **正确性（M4）先于性能（M5）**：与 AGENTS.md 的验证观一致——先证明 batched
  MTP 自身状态机、容量、stop 与统计语义，再做 graph 与 small-T 调优；调优必须以
  profiling 证据立项。
- **不再细分**：每个里程碑是一个可独立验证的行为边界（工件可加载 / 算子族数值
  正确 / MTP 前向对拍 / verify 窗口自洽 / MTP E2E 行为 / 实测加速）。里程碑内部的
...
1. **small-T verify 成本（最大性能风险）**：若 T=6 verify 成本远超 T=1（generic
   ...
   排序调优项。
2. **MTP 自洽性**：batched verify 与普通 decode 可因浮点路径差异产生不同 token
   序列；M4 只验收 MTP committed 状态与自身 verify/accept 结果一致，不把
   MTP-on/off 输出相等作为 gate。

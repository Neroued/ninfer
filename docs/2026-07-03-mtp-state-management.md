...
## 4. Target GDN 状态：snapshot 槽位 + selector 间接读取

### 4.1 缓冲布局（取代现有单份状态）

每个 GDN 层持有一个 conv 大 tensor 和一个 ssm 大 tensor，逻辑槽位数
`Slots = k+1`：

```text
conv_states[l]: [10240, 3, Slots]     bf16    # slot j = [:, :, j]
ssm_states[l]:  [128, 128, 48, Slots] fp32    # slot j = [..., j]
gdn_initial_slot: [1] i32 device scalar
```

- 定义：round 之间，与 committed 前缀 `0..L-1` 一致的 GDN 状态位于
  `gdn_initial_slot` 指定的槽位。
- conv 与 ssm 必须共用同一个 selector，不能分别选择。
- `GdnState::reset` 清零 slot 0；prefill reset 同时把 `gdn_initial_slot` 置 0。
- chunked prefill 与 MTP-disabled 普通 decode 一律读写 slot 0，行为与今天一致。
- MTP-enabled fallback decode 从 `gdn_initial_slot` 读初始状态，写 after-token 状态到
  slot 0，然后将 selector 重置为 0。

### 4.2 verify 阶段的写入规则

verify 处理 `T_v = k+1` 个 token 时，conv/recurrent 算子以
`slot[gdn_initial_slot]` 为初始状态，在处理完第 `j` 个 token 后把状态快照写入
slot `j`（`j = 0..k`）：
...
### 4.3 accept 后 selector 更新

accept 之后、下一次 GDN 读之前，执行一次 device-scalar 赋值：

```text
gdn_initial_slot := a
```

无需在 accept 后复制 conv/ssm 状态。下一轮 verify 的 kernel 根据 selector 读取
对应槽位；下一次 MTP-enabled fallback decode 也使用相同 selector，并在写回 slot 0
后重置 selector。
...
之后的终态），予以取代。以下契约中 `Slots = k+1`，`T ≤ Slots`；算子不理解
accept，只做纯序列变换 + 快照。
...
causal_conv1d_sequence_snapshot(
    ...
    weight:  [10240, 4] BF16,
    states:  [10240, 3, Slots] BF16 in-out,
    initial_slot: [1] I32 device scalar,
    out:     [10240, T] BF16 compact)

语义: 初始窗口 = states[:,:,safe(initial_slot)]（先读入寄存器）;
...
gated_delta_rule_recurrent_snapshot(
    ...
    scale: 1/sqrt(128),
    states: [128,128,48,Slots] FP32 in-out,
    initial_slot: [1] I32 device scalar,
    out:    [128,48,T] BF16 compact)

语义: 初始 state = states[...,safe(initial_slot)]（先读入寄存器/局部）;
...
`gdn_initial_slot` 是 `[1] I32` device scalar。accept kernel 写出 `a` 后，专用小
kernel 将其复制为下一次 GDN selected-state 读取的初始槽位。prefill reset 与
MTP-enabled fallback decode 结束后将其置 0。
...
| `a` / `ar_pos` / `gdn_initial_slot` | i32 device 标量 | 轮内/跨轮 |
...
1. round 之间 GDN committed 状态由 `gdn_initial_slot` 指定；MTP KV 的 `pos` 表示
   committed MTP 前缀长度。
2. 状态调试应检查 selector 指向槽位、MTP KV cursor、accept 数与 host 位置镜像是否

...
状态：**待评审的 v4.1 提案**。本文件给出格式变更和实施顺序；在 §8 的格式冻结门槛通过前，
`docs/q5090_packed_file_format_v4.md` 中的 v4.0 仍是当前实现的事实合同。
...
- runtime 必须验证 CRC32、UTF-8、JSON/merges 语义和全部 tokenizer padding；SHA-256 可在正常加载
  中验证，若为了避免引入新的 crypto dependency 暂不验证，也必须保留并由 offline verifier 强制；
- `generation_config.json` 在 v4.1 中是必需 asset，不再允许缺失时硬编码 `{248046, 248044}`；
- `merges.txt` 在 v4.1 中是必需 asset，不再允许先构造成功、首次 encode 时才报错；
- tokenizer 只驻留 CPU，不进入 GPU load plan、DeviceArena 或 H2D byte 统计；派生表构造完成后可释放
  raw bundle。
...
5. runtime 读取 tokenizer 后强制 CRC/UTF-8/内容解析；offline verifier 额外强制 SHA-256 和所有
   tokenizer/payload padding zero-fill。
...
- **runtime structural validation**：只依赖 header/catalog/tokenizer，防止错误寻址和 token identity
  错误；
...
converter 可以从 `--tokenizer <dir>` 取 tokenizer 输入，默认可指向同一 HF model dir；但三份 asset
必须全部存在。该选项只存在于转换工具，之后从 runtime CLI/serve options 中删除。
...
5. `qus`、serve、benchmark 和其他所有入口只接收一个 `.qus`；需要文本处理的入口从同一 artifact
   获取 tokenizer，不需要 tokenizer 的 benchmark 不解析派生表；
...
host descriptor 按 module 使用独立、不可移动的 record slab。新增 optional module 不得导致已绑定
TEXT descriptor 因 `std::vector` 扩容而换地址。
...
每个 module 的加载遵循 prepare -> upload -> validate -> publish：
...
- recoverable error 且 stream/context 健康时只回滚当前未发布 module；
...
- tokenizer record 缺失/重复/乱序、非法 encoding/UTF-8、CRC/SHA、非零 padding；
...
- deterministic conversion：相同输入产生相同 header/catalog/tokenizer hashes 和 weight CRC。
...
- §5.4 四种 request 的 actual file-read/H2D/resident bytes 精确匹配 plan；
- non-resident lookup 为 null，但 catalog available 状态正确；
- optional module 加载/失败不改变已绑定 TEXT descriptor/device pointer；
- planned offset 256-byte alignment 和 arena bounds 全部断言；
- graph capture 只在 startup residency 完成后发生。

### 12.4 生命周期和性能

- compact fixture 在 `compute-sanitizer` 下执行 staged upload + bind smoke；
- 注入 short read、callback throw、enqueue/event failure，证明先 drain 再释放；

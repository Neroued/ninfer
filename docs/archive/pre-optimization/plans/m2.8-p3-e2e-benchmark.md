...
int test_raw_report_json_is_valid() {
    ...
    failures += text.find("\"prefill_time_s_median\"") != std::string::npos &&
                        text.find("\"decode_tok_s_median\"") != std::string::npos &&
    ...
    failures += text.find("\"decode_tok_s_valid\": true") != std::string::npos
                    ? 0
                    : fail("decode validity missing");
    failures += text.find("\"decode_tok_s\": null") != std::string::npos &&
                        text.find("\"decode_tok_s_valid\": false") != std::string::npos
                    ? 0
...
when `RepeatReport::decode_tok_s_valid()` is false.
...
engine.decode_metric = "decode_tok_s"
...
- Primary timing calls `Engine::load()`, `Engine::prefill()`, and `Engine::decode_step()` directly, not
  `Engine::generate()`.
- `decode_tok_s` includes per-step token D2H readback because timing surrounds `decode_step()`.
...
- environment fields record CUDA runtime version, CUDA driver version, GPU name, and selected device when
  available.
- `decode_loop_tokens == 0` emits `decode_tok_s: null` and
  `decode_tok_s_valid: false`.

...
- `profiles/m3-p2-gemv-task4/round1-q4-mlp-gate-up/baseline_stalls.ncu.{rep,csv,txt}`
- `profiles/m3-p2-gemv-task4/round1-q4-mlp-gate-up/baseline_instr.ncu.{rep,csv,txt}`
- `profiles/m3-p2-gemv-task4/round1-q4-mlp-gate-up/baseline_source.ncu.{rep,csv,txt}`
- `profiles/m3-p2-gemv-task4/round1-q4-mlp-gate-up/baseline_sched.ncu.{rep,csv,txt}`
...
- `profiles/m3-p2-gemv-task4/round1-q4-mlp-gate-up/after_stalls.ncu.{rep,csv,txt}`
- `profiles/m3-p2-gemv-task4/round1-q4-mlp-gate-up/after_instr.ncu.{rep,csv,txt}`
- `profiles/m3-p2-gemv-task4/round1-q4-mlp-gate-up/after_source.ncu.{rep,csv,txt}`
- `profiles/m3-p2-gemv-task4/round1-q4-mlp-gate-up/after_sched.ncu.{rep,csv,txt}`

### Diagnosis

The Q4 baseline confirmed the dominant limiter was uncoalesced global access through L1TEX, not
spills or missing occupancy. DRAM was only 15.47% while Memory SOL was 92.33%, L1/TEX throughput was
98.14%, achieved occupancy was 90.09%, and the kernel had zero local/shared spilling. Scheduler
metrics showed only 0.18 eligible warps per scheduler, with long scoreboard at 60.18% of active-warp
issue stalls and LG throttle at 28.40%. Source Counters reported 51,423,232 excessive sectors, 88%
of 58,508,288 total sectors, matching the old warp-per-row schedule where lanes touched different
K64 groups at a 2176-byte Q4 tile stride.
...
| metric | before | after |
| --- | ---: | ---: |
| NCU duration | 185.31 us | 134.21 us |
| DRAM throughput | 15.47% | 21.43% |
| Memory SOL | 92.33% | 30.96% |
| Compute SOL | 12.68% | 35.58% |
| L1/TEX throughput | 98.14% | 41.58% |
| achieved occupancy | 90.09% | 96.41% |
| registers/thread | 35 | 28 |
| spills/local memory | 0 | 0 |
| long scoreboard | 60.18% | 70.67% |
| second stall | LG throttle, 28.40% | wait, 10.62% |
| LG throttle | 28.40% | 0.02% |
| MIO throttle | 3.80% | 0.08% |
| eligible warps/scheduler | 0.18 | 1.28 |
| issue active | 13.51% | 47.85% |
| global-load instructions | 2,193,408 | 5,570,560 |
| global-load L1TEX sectors | 58,490,880 | 9,748,480 |
| global-load data bytes | 1.87 GB | 311.95 MB |
| DRAM read bytes | 47.38 MB | 47.37 MB |
| total SM instructions | 32,204,800 | 65,140,736 |
| source excessive sectors | 51,423,232 | no uncoalesced-access warning |
...
dropped by 83.33%, the uncoalesced global-access warning disappeared, LG throttle effectively went
to zero, eligible warps per scheduler rose from 0.18 to 1.28, NCU duration improved by 27.58%, and
DRAM throughput rose by 5.96 points with no spills or correctness regression. This still does not
meet the Task 4 target (`>=70%` DRAM and near `C = 82.76%`). The tradeoff is higher total instruction
count and the next limiter is now long-scoreboard/wait latency in the row-group schedule rather than
sector inflation.

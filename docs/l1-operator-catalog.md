...
- **`gated_delta_rule`:** the recurrence. `recurrent` (decode, `T=1`, §7.1) updates `ssm_state` in place and does not allocate workspace; `chunked` (prefill, §7.2, chunk 64, WY/UT) uses `ws` for native-bf16 stage scratch. `scale = 1/√128` (applied to `q`). GVA mapping (v-head `h` uses k/q head `h//3`) is internal. Replaces arch §10 `gated_delta_recurrent/chunked`.

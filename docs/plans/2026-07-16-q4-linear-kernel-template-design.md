...
- Q4 `[131072,5120]` 的 SIMT TT4 比专门 TT1 更快；
- `[34816,5120]` 在 Cols 8/9 之间切换 SIMT/MMA；
- 两个 Vision shape 分别在不同 Cols 区域切换；
- 因此不能用场景名或一个全局 token 阈值决定 lifecycle。
...
要求整除，也保证区间起点、终点和 scale load 都保持 4B pair 对齐。静态要求
`GroupsPerWarpTile` 为正偶数。一个运行时 K 实例可以合法覆盖：
...
最后一个不足 `GroupsPerWarpTile` 的 tile 仍按完整 Q4 group 的 32B code 向量加载，
scale 始终以 4B pair 加载；consumer 只访问 active group 对应的 X。不存在 2B
`cp.async`、inactive X load 或逐元素 K tail。
...
```text
GemvR4W1G16    : RowsPerCta=4, WarpsPerRow=1, GroupsPerWarpTile=16
GemvR1W8G16    : RowsPerCta=1, WarpsPerRow=8, GroupsPerWarpTile=16
```
...
每个枚举值不可变地绑定一个完整 schedule。首期候选的完整事实是：

| ScheduleId | 完整 codegen schedule |
|---|---|
| `GemvR4W1Shared` | R4/W1/G16/S1，`CtaSharedFullK`，code `Ca`，launch-bounds min blocks 1 |
| `GemvR4W1Direct` | R4/W1/G16/S1，`Direct`，code `Ca`，launch-bounds min blocks 1 |
| `GemvR1W8Direct` | R1/W8/G16/S1，`Direct`，code `Ca`，launch-bounds min blocks 1 |
...
```text
q4.gemv.r4.w1.g16.s1.x_shared_full_k.code_ca.lb1
...
```text
GEMV      2 个最终 ownership schedule，外加最多 1 个 direct/shared 比较候选
SIMT      2 column schedules x 必要 variant
MMA       2 column schedules x Full/Predicated
```

资格收敛后约 8～11 个 CUDA entry。这个规模是有意的：
...
capability 描述一个 `Q4ScheduleId + Q4KernelVariant` 的物理合法性。真实实现优先使用
constexpr predicate：
...
相关，capability 必须在 resolver 之前证明 `dynamic_shared + static_shared` 不超过该
schedule 的 launch 上限。`Direct` 实例可以使用不同的 K envelope。
...
### 10.4 当前 route 假设，不是最终资格结论

已有证据给出以下强候选：

| Shape | 已知候选结构 |
|---|---|
| `[4096,5120]` Cols1 | split-K8 GEMV |
| `[6144,5120]` Cols1 | SIMT C4 |
| `[1024,5120]` Cols1 | SIMT C4 |
| `[34816,5120]` Cols1 | warp-row GEMV |
| `[131072,5120]` Cols1 | SIMT C4 |
| `[34816,5120]` Cols2..8 | SIMT C4/C8 |
| `[34816,5120]` Cols9+ | MMA candidate |
| `[3456,1152]` Cols24 | SIMT winner anchor |
| `[3456,1152]` Cols28 | MMA winner anchor |
| `[4304,1152]` Cols12 | SIMT/BN128 practical tie anchor |
| `[4304,1152]` Cols16 | MMA winner anchor |
| future `[131072,2048]` Cols1..8 | SIMT C4 |
| future `[131072,2048]` Cols9..64 | MMA C64 |
| future `[131072,2048]` Cols65+ | MMA C128 |

实现切换前必须用新 binary 重新测量：

- Cols4/5 的 SIMT C4/C8 边界；
- Vision crossover 中间整数；
- MMA C64/C128 边界；
- 新 GEMV template 与旧 tuned kernel；
- full/predicated variant；
- K1152 group-edge 路径。

旧 profiler 文件和旧 build binary 不能替代新源码的资格测量。
...
当前源码中的 `ninfer_linear_op_bench` 没有 fixed Q4 candidate、Vision shape 或任意 N/K
入口；现有 `build/` 中带 candidate 选项的二进制来自已放弃源码，时间也早于当前源文件。
它不能继续用于资格实验。

第一次新测量前必须完成：

1. 从当前源码重新构建；
2. 增加只在 `src/` 内可见的 fixed candidate 入口；
3. benchmark 支持 `Rows/K/Cols` 以及 `legacy/new/auto` candidate；
4. `new` 使用 canonical `Q4ScheduleId + Q4KernelVariant`，`legacy` 使用独立 measurement-only
   identity，不污染 production enum；
5. CSV 写出 canonical ScheduleId、KernelVariant、build type、CUDA 和 GPU；
6. correctness helper 根据实际 forced/resolved ScheduleId 选择 tolerance；
7. 用 `--help` 和一个 smoke CSV 证明运行的是新 binary。

在该门完成前只能引用历史结果作为设计证据，不能追加新的资格结论。

...
| Status | Exact result |
|---|---|
| **Supported** | N5 Vision LayerNorm `[1152,4096]`, E1 Vision QKV bias add `[3456,4096]`, E6 tanh GELU `[4304,4096]`, A4 one-segment Vision attention `[72,16,4096]`, and S3/S4 GDN recurrence and snapshot state transition `[128,16,32]`. |
| **Adapt existing** | Every target-callable dense/quantized Linear domain plus private L15-L17; indexing transforms; all Text/GDN/MTP normalization; pointwise domains other than E1/E6; Text and Vision RoPE; all 35B GQA routes; GDN convolution; generation and MTP-control Ops. All remain unsupported. |

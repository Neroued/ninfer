...
The explicit per-column `SparseMoe` route is a temporary functionality bootstrap using the
performance-qualified specialized single-column kernels, not a generic Linear fallback. Its
availability is recorded below, but it does not satisfy the grouped multi-column performance
requirements. `SparseMoe` itself accepts any positive T; target schedule bounds do not become Op
bounds.
...
|---|---|---|---|---|
| L1 | Multi-output Linear; W8 weight `[9216,2048]`, `X[2048,T]` | 10 Text full-attention layers and one MTP layer | `Y=W X`; rows `0:4096`, `4096:4608`, `4608:8704`, and `8704:9216` produce Q `[256,16,T]`, K `[256,2,T]`, gate `[256,16,T]`, and V `[256,2,T]`. | **Functionally supported for every positive T; performance evidence covers measured points through the default `T=1024` target.** One contiguous W8 parent Linear exposes Q/K/gate/V as zero-work row views. At the measured points its private SIMT/MMA route reaches 176.42 TFLOP/s, or 83.79% of the same-session BF16 MMA probe, with no spilling. See the [retained RTX 5090 report](archive/optimization-era/bench/qwen3.6-35b-linear-roofline.md). |
| L2 | Multi-output Linear; W8 weight `[12288,2048]`, `X[2048,T]` | 30 GDN layers | `Y=W X`; row widths `2048,2048,4096,4096` produce Q `[128,16,T]`, K `[128,16,T]`, V `[128,32,T]`, and Z `[128,32,T]`. | **Functionally supported for every positive T; performance evidence covers measured points through the default `T=1024` target.** One contiguous W8 parent Linear exposes Q/K/V/Z as zero-work row views. At the measured points it reaches 169.31 TFLOP/s, or 85.36% of the same-session BF16 MMA ceiling, with no spilling. See the [retained RTX 5090 report](archive/optimization-era/bench/qwen3.6-35b-linear-roofline.md). |
...
|---|---|---|---|---|
| L15 | Private Linear; BF16 weight `[257,2048]`, BF16 `X[2048,T]` -> logical scores `[257,T]` | `T=1` is qualified; larger T needs grouped-performance work | Rows `0:256` are router logits and row 256 is the independent shared-expert score. | **The single-column kernel is performance-qualified; every positive T is functional.** The 257-CTA, 8-warp kernel writes private FP32 scores and sits on its same-grid payload floor. The current implementation applies it independently to every column; grouped performance remains unsupported. See the [retained single-column report](archive/optimization-era/bench/qwen3.6-35b-sparse-moe-decode-roofline.md). |
| L16 | Private LinearSwiGLU; W8 weight `[1024,2048]`, BF16 `X[2048,T]` -> logical activation `[512,T]` | `T=1` is qualified; larger T needs grouped-performance work | Split `W X` into gate/up `[512,T]`; return `SiLU(gate).*up` for the always-on shared expert. | **The single-column kernel is performance-qualified; every positive T is functional.** D3 computes the shared W8 pair with the selected routed experts and retains its natural FP32 result. Column iteration is correct but does not qualify grouped weight reuse. |
| L17 | Private Linear; W8 weight `[2048,512]`, logical `X[512,T]` -> logical result `[2048,T]` | `T=1` is qualified; larger T needs grouped-performance work | `Y_shared=W_shared_down X`. | **The single-column kernel is performance-qualified; every positive T is functional.** The exact W8 K=512 path is fused with routed down, merge, and `AddResidual` in D4. Grouped performance remains unsupported. |
| L18 | Private expert-grouped LinearSwiGLU; per expert `W_gu[e][1024,2048]`, BF16 `X_e[2048,M_e]` -> logical activation `[512,M_e]` | Every positive T is functional; grouped multi-column performance is unqualified | For every active expert, split `W_gu[e]X_e` into 512-row gate/up halves and return `SiLU(gate_e).*up_e`. | **Single-column expert work is performance-qualified; the complete formula is functional for every positive T.** Column-serial D3 directly addresses every selected Q4 or W8 bank without dropping assignments, but device-described grouping and cross-column weight reuse remain new work. |
| L19 | Private expert-grouped Linear; per expert `W_down[e][2048,512]`, logical `X_e[512,M_e]` -> logical result `[2048,M_e]` | Every positive T is functional; grouped multi-column performance is unqualified | `Y_e=W_down[e]X_e` for every active expert. | **Single-column expert work is performance-qualified; the complete formula is functional for every positive T.** Column-serial D4 covers exact Q5, Q6, and W8 K=512 rows, but grouped contractions remain new work. |
...
|---|---|---|---|
| M1 | Router softmax, top-8, selected-weight renormalization, and shared sigmoid | Consume L15's logical router scores `[256,T]` and independent shared score `[T]`; produce I32 ids `[8,T]`, logical route weights `[8,T]`, and a logical shared scale `[T]`. Select the eight largest softmax probabilities per token using the stated tie rule and renormalize only those eight; shared scale is the sigmoid of the independent score. Private storage precision is selected by the implementation. | **The single-column kernel is performance-qualified; every positive T is functional.** The implementation applies exact lower-id-stable selection and selected-logit normalization independently to every column. A grouped selection route remains unqualified. |
| M2 | Assignment grouping and inverse-map construction | Form assignments `(expert,token,route_slot,weight)` for `A=8T`; partition them into expert jobs of size `M_e` while preserving a bijection back to token/route order. | **New for optimized multi-column execution.** The serial implementation completes each token before advancing and therefore needs no physical multi-token jobs or inverse map; it does not qualify grouped performance. |
| M3 | Inverse scatter, route-weighted reduction, shared merge, and output epilogue | Restore token order and compute `Y[:,t]=sum_r alpha[r,t]*Y_r[:,t]+shared_scale[t]*Y_shared[:,t]`; in the required `AddResidual` mode, finish with `R[:,t]'=R[:,t]+Y[:,t]`. The mode affects only this final writeback. | **The single-column kernel is performance-qualified; every positive T is functional.** D4 performs FP32 routed/shared scaling and reduction plus the sole BF16 write independently for every column. A grouped inverse map remains unsupported. |
| M4 | Closed SparseMoe Op | Compose private Linear stages L15-L19 and sparse stages M1-M3 for every positive T, with the required `AddResidual` epilogue and all workspace effects owned by one Op call. | **Functionally complete for every positive T; only the single-column execution is performance-qualified.** One repository-internal API and constant-capacity workspace apply the same route independently per column. Codec selection and token count are independent. Grouped multi-column performance and the 35B target remain unsupported. |
...
| **Single-column performance-qualified** | Private L15-L19 and sparse M1/M3/M4 at `T=1`, including Q4+Q5, Q4+Q6, and W8+W8, direct selected-bank addressing, the required `AddResidual` epilogue, caller-owned graph-stable workspace, and complete-Op oracle/roofline evidence. M2 degenerates to the eight associated route pairs and needs no separate representation at this point. |
| **Functional but not multi-column performance-qualified** | M4 and its private L15-L19/M1/M3 work accept every positive T through stream-ordered reuse of the complete single-column route. Codec profiles are orthogonal to token count. The iterator passes the common oracle with distinct multi-column inputs but has no grouped performance claim. |
| **Adapt existing** | The remaining multi-column performance work for private L15-L17. |
| **New implementation** | Grouped multi-column L18-L19, M2 assignment grouping/inverse maps, and the remaining optimized multi-column portions of M1/M3/M4. |

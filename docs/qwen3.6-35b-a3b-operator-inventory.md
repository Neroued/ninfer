...
`SparseMoe` has production decode, Small-T, and grouped prefill routes. The Small-T implementation
admits `T=2..32`; codec-specific trace-like crossovers move Q4+Q5, Q4+Q6, and W8+W8 to prefill at
T=12, T=11, and T=7. Prefill slices at 4096, so `SparseMoe` accepts every positive T without a
per-column fallback. Performance qualification remains limited to the explicit boundary and
throughput points below; target schedule bounds do not become Op bounds.
...
|---|---|---|---|---|
| L15 | Private Linear; BF16 weight `[257,2048]`, BF16 `X[2048,T]` -> logical scores `[257,T]` | Decode, codec crossover neighborhoods, and `T=1024` are performance-qualified; every positive T is functional | Rows `0:256` are router logits and row 256 is the independent shared-expert score. | T1 uses the 257-CTA decode kernel. Small-T exact-T templates divide K into four partitions and reuse each router row across columns. Prefill uses BF16 Tensor Core MMA over all columns and feeds device selection/counting. See the retained [decode](archive/optimization-era/bench/qwen3.6-35b-sparse-moe-decode-roofline.md) and historical [Small-T](archive/optimization-era/bench/qwen3.6-35b-sparse-moe-small-t.md) reports. |
| L16 | Private LinearSwiGLU; W8 weight `[1024,2048]`, BF16 `X[2048,T]` -> logical activation `[512,T]` | Complete-route performance is qualified at the codec transitions and `T=1024`; every positive T is functional | Split `W X` into gate/up `[512,T]`; return `SiLU(gate).*up` for the always-on shared expert. | Decode and Small-T fuse the shared path into D3. Prefill uses the shared W8 BF16 MMA gate/up contraction and aliases its BF16 activation with expired router-score storage. |
| L17 | Private Linear; W8 weight `[2048,512]`, logical `X[512,T]` -> logical result `[2048,T]` | Complete-route performance is qualified at the codec transitions and `T=1024`; every positive T is functional | `Y_shared=W_shared_down X`. | Decode and Small-T fuse the shared path into D4. Prefill's W8 down contraction consumes the shared activation and performs shared scaling, routed merge, residual add, and the sole BF16 output write. |
| L18 | Private expert-grouped LinearSwiGLU; per expert `W_gu[e][1024,2048]`, BF16 `X_e[2048,M_e]` -> logical activation `[512,M_e]` | Complete-route performance is qualified at Small-T transitions, `T=1024`, and larger Q4+Q5 points; every positive T is functional | For every active expert, split `W_gu[e]X_e` into 512-row gate/up halves and return `SiLU(gate_e).*up_e`. | Small-T directly addresses selected banks in one persistent D3 launch. Prefill gathers columns by expert and executes flattened Q4 BF16-MMA jobs or exact W8 expert tiles without selected-weight repacking. |
| L19 | Private expert-grouped Linear; per expert `W_down[e][2048,512]`, logical `X_e[512,M_e]` -> logical result `[2048,M_e]` | Complete-route performance is qualified at Small-T transitions, `T=1024`, and larger Q4+Q5 points; every positive T is functional | `Y_e=W_down[e]X_e` for every active expert. | Small-T directly addresses Q5/Q6/W8 banks in one persistent D4 launch. Prefill executes flattened Q5/Q6 BF16-MMA jobs or exact W8 expert tiles and writes packed expert outputs for inverse reduction. |
...
|---|---|---|---|
| M1 | Router softmax, top-8, selected-weight renormalization, shared sigmoid, and prefill counts | Consume L15's logical router scores `[256,T]` and independent shared score `[T]`; produce I32 ids `[8,T]`, logical route weights `[8,T]`, and a logical shared scale `[T]`. Select the eight largest softmax probabilities per token using the stated tie rule and renormalize only those eight; shared scale is the sigmoid of the independent score. Private storage precision is selected by the implementation. | Decode and Small-T apply the exact lower-id tie rule independently per token. Prefill additionally counts each tile's assignments per expert on device for M2; every positive T is functional. |
| M2 | Assignment grouping and inverse-map construction | Form assignments `(expert,token,route_slot,weight)` for `A=8T`, partition them into expert jobs of size `M_e`, and preserve a bijection back to token/route order. | Small-T needs no physical grouping. Prefill scans device counts into expert offsets and tile bases, gathers BF16 columns into expert-major order, creates flattened 32- or 64-column jobs, and replaces the local rank with the inverse packed-column map. |
| M3 | Inverse scatter, route-weighted reduction, shared merge, and output epilogue | Restore token order and compute `Y[:,t]=sum_r alpha[r,t]*Y_r[:,t]+shared_scale[t]*Y_shared[:,t]`; in the required `AddResidual` mode, finish with `R[:,t]'=R[:,t]+Y[:,t]`. The mode affects only this final writeback. | Decode and Small-T make token mapping implicit in D4. Prefill follows the inverse map, reduces eight BF16 routed outputs with FP32 route weights into FP32 token sums, and combines them with shared down and the residual in the final BF16 store. |
| M4 | Closed SparseMoe Op | Compose private Linear stages L15-L19 with M1-M3 for every positive T, with the required `AddResidual` epilogue and all workspace effects owned by one Op call. | Functionally complete for every positive T with three production regimes: T1 decode; Small-T below the Q4+Q5/Q4+Q6/W8+W8 crossovers 12/11/7; grouped Tensor Core prefill from those points upward, using 4096-column slices. Complete-Op evidence covers the transition neighborhoods, all codecs at the wide-plan boundary, the primary T=1024 workload, and larger Q4+Q5 points. |
...
| **SparseMoe performance-qualified points** | Private L15-L19 and sparse M1-M4 have complete-Op evidence at T1, the codec-specific Small-T/prefill transition neighborhoods, all codecs at T768/T1024, and Q4+Q5 at T2048/T4096. Evidence includes realistic distinct per-token routes, device grouping/inverse mapping, direct persistent-weight access, `AddResidual`, caller-owned graph-stable workspace, the complete oracle, multi-run timing, and NCU attribution. At T1024 the complete Q4+Q5 Op takes about 813 microseconds and sustains 72.6 useful TFLOP/s. |
| **Functionally supported larger extents** | M4 accepts every larger positive T with at most 4096 columns per prefill slice. T4097 correctness and Graph replay exercise the slice boundary; unmeasured extents do not inherit a roofline claim. Codec profiles remain orthogonal to token count. |
| **Remaining performance work** | Further prefill tuning may improve achieved hardware utilization, but no serial or generic functional fallback remains in the registered token domain. |

The registered 35B target composes these Ops through its family runtime and exact Variant. Sparse
MoE optimization changes only the repository-internal dispatch/workspace implementation and does
not alter target registration, artifact storage, or the public Op formula.

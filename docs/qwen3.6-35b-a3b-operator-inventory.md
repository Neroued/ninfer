...
|---|---|---|---|---|
| L15 | Private Linear; BF16 weight `[257,2048]`, BF16 `X[2048,T]` -> logical scores `[257,T]` | All three T regimes | Rows `0:256` are router logits and row 256 is the independent shared-expert score. | **Supported for decode `T=1` only.** The qualified 257-CTA, 8-warp row route writes private FP32 scores and sits on its same-grid payload floor. Small-T and prefill remain unsupported. See the [retained decode report](archive/optimization-era/bench/qwen3.6-35b-sparse-moe-decode-roofline.md). |
| L16 | Private LinearSwiGLU; W8 weight `[1024,2048]`, BF16 `X[2048,T]` -> logical activation `[512,T]` | All three T regimes | Split `W X` into gate/up `[512,T]`; return `SiLU(gate).*up` for the always-on shared expert. | **Supported inside `SparseMoe` for decode `T=1` only.** D3 computes the shared W8 gate/up pair in the same nine-path launch as the selected routed experts and retains its natural FP32 SwiGLU result. Small-T and prefill remain unsupported. |
| L17 | Private Linear; W8 weight `[2048,512]`, logical `X[512,T]` -> logical result `[2048,T]` | All three T regimes | `Y_shared=W_shared_down X`. | **Supported inside `SparseMoe` for decode `T=1` only.** The exact W8 K=512 path is fused with routed down, merge, and `AddResidual` in D4. Small-T and prefill remain unsupported. |
| L18 | Private expert-grouped LinearSwiGLU; per expert `W_gu[e][1024,2048]`, BF16 `X_e[2048,M_e]` -> logical activation `[512,M_e]` | Decode `M_e in {0,1}`; Small-T `sum M_e<=48`; prefill `sum M_e<=8192` | For every active expert, split `W_gu[e]X_e` into 512-row gate/up halves and return `SiLU(gate_e).*up_e`. | **Supported for decode `M_e in {0,1}` only.** One D3 launch directly addresses the selected Q4 or W8 banks and the shared expert. Device-described Small-T and prefill grouping remain new work. |
| L19 | Private expert-grouped Linear; per expert `W_down[e][2048,512]`, logical `X_e[512,M_e]` -> logical result `[2048,M_e]` | Same three regimes | `Y_e=W_down[e]X_e` for every active expert. | **Supported for decode `M_e in {0,1}` only.** Exact Q5, Q6, and W8 K=512 specializations directly reduce eight selected banks in D4. Small-T and prefill grouped contractions remain new work. |
...
|---|---|---|---|
| M1 | Router softmax, top-8, selected-weight renormalization, and shared sigmoid | Consume L15's logical router scores `[256,T]` and independent shared score `[T]`; produce I32 ids `[8,T]`, logical route weights `[8,T]`, and a logical shared scale `[T]`. Select the eight largest softmax probabilities per token using the stated tie rule and renormalize only those eight; shared scale is the sigmoid of the independent score. Private storage precision is selected by the implementation. | **Supported for decode `T=1` only.** One warp selects the exact lower-id-stable top-8, normalizes only those selected logits in FP32, and computes the independent shared sigmoid. Small-T and prefill remain unsupported. |
| M2 | Assignment grouping and inverse-map construction | Form assignments `(expert,token,route_slot,weight)` for `A=8T`; partition them into expert jobs of size `M_e` while preserving a bijection back to token/route order. | **New for Small-T and prefill.** Decode grouping degenerates to the eight associated `(id,weight)` pairs produced by M1 and requires no job or inverse-map representation. No multi-token grouping path exists. |
| M3 | Inverse scatter, route-weighted reduction, shared merge, and output epilogue | Restore token order and compute `Y[:,t]=sum_r alpha[r,t]*Y_r[:,t]+shared_scale[t]*Y_shared[:,t]`; in the required `AddResidual` mode, finish with `R[:,t]'=R[:,t]+Y[:,t]`. The mode affects only this final writeback. | **Supported for decode `T=1` only.** D4 directly performs all routed/shared down dots, FP32 scaling and reduction, residual read, and the sole BF16 write. Multi-token inverse mapping remains unsupported. |
| M4 | Closed SparseMoe Op | Compose private Linear stages L15-L19 and sparse stages M1-M3 for decode T=1, Small-T `T<=6`, and prefill `T<=1024`, with the required `AddResidual` epilogue and all workspace effects owned by one Op call. | **Supported for decode `T=1` only.** The repository-internal Op, exact workspace query, four-launch Q4+Q5/Q4+Q6/W8+W8 route, complete independent oracle, CUDA Graph replay, and RTX 5090 qualification are implemented. The API rejects Small-T and prefill, so the complete M4 domain and 35B target remain unsupported. |
...
| **Supported for decode only** | Private L15-L19 and sparse M1/M3/M4 at exactly `T=1`, including Q4+Q5, Q4+Q6, and W8+W8, direct selected-bank addressing, the required `AddResidual` epilogue, caller-owned graph-stable workspace, and complete-Op oracle/roofline evidence. M2 degenerates to the eight associated route pairs and needs no separate decode representation. |
| **Adapt existing** | The remaining Small-T and prefill portions of private L15-L17. |
| **New implementation** | Small-T/prefill grouped L18-L19, M2 assignment grouping/inverse maps, and the remaining multi-token portions of M1/M3/M4. |

No complete Text/GDN block, full-domain sparse-MoE execution, generation path, or MTP execution
currently meets the support definition for the 35B target. Decode-only `SparseMoe` support does not
register that target.

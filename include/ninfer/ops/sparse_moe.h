...
/**
 * Returns the transient capacity required by the registered SparseMoe route for every admitted
 * token count in [1,max_tokens]. The first registered route admits decode only, so max_tokens must
 * be exactly one.
 */
[[nodiscard]] std::size_t sparse_moe_workspace_bytes(std::int32_t max_tokens);

/**
 * Closed sparse-MoE decode Op for the exact future 35B-A3B geometry.
 *
 * For contiguous BF16 x [2048,1], destination [2048,1], 256 routed experts, top-8 selection, and
 * one always-on shared expert, this Op owns router projection and selection, selected routed and
 * shared SwiGLU projections, down projections, their merge, and the AddResidual epilogue. At an
 * exact top-8 boundary tie the lower expert id wins. destination is the only observable mutation:
 * its incoming value is the residual and its outgoing value is the BF16 sparse-MoE result plus
 * that residual.
 *

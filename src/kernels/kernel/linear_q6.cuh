...
// qus::kernels - Q6G64 TILE_N64_K64 linear kernels. Correctness-baseline
...
inline constexpr std::int32_t kLinearQ6Group = 64;
inline constexpr std::int32_t kLinearQ6Bpr   = 48;
inline constexpr std::int32_t kLinearQ6Tilew = 64 * 2 + 64 * kLinearQ6Bpr;

__device__ __forceinline__ int unpack_q6_code(const std::uint8_t* packed, int lane) {
    std::uint32_t u  = 0;
    const int bitpos = lane * 6;
    for (int b = 0; b < 6; ++b) {
...
__device__ __forceinline__ float load_q6_weight(const std::uint8_t* payload, std::int32_t row,
                                                std::int32_t group, std::int32_t lane,
                                                std::int32_t kg) {
    const std::int32_t tile        = row / 64;
    const std::int32_t row_in_tile = row - tile * 64;
    const std::int64_t off =
        (static_cast<std::int64_t>(tile) * kg + group) * static_cast<std::int64_t>(kLinearQ6Tilew);
    ...
    const std::uint8_t* packed =
        payload + off + 64 * 2 + static_cast<std::int64_t>(row_in_tile) * kLinearQ6Bpr;
    return static_cast<float>(unpack_q6_code(packed, lane)) * scale;
}

__global__ void linear_q6_gemv_kernel(const __nv_bfloat16* x, const std::uint8_t* payload,
                                      __nv_bfloat16* out, std::int32_t n, std::int32_t k,
                                      std::int32_t padded_k) {
    const std::int32_t kg     = padded_k / kLinearQ6Group;
    ...
    for (std::int64_t row64 = start; row64 < n; row64 += stride) {
        ...
        for (std::int32_t group = 0; group < kg; ++group) {
            for (std::int32_t lane = 0; lane < kLinearQ6Group; ++lane) {
                const std::int32_t kk = group * kLinearQ6Group + lane;
                if (kk >= k) { break; }
                const float wv = load_q6_weight(payload, row, group, lane, kg);
...
__global__ void linear_q6_gemm_kernel(const __nv_bfloat16* x, const std::uint8_t* payload,
                                      __nv_bfloat16* out, std::int32_t n, std::int32_t k,

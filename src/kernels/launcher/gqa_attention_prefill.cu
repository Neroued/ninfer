...
void gqa_attention_prompt_attention_launch(const Tensor& q, const Tensor& positions, float scale,
    ...
    static const cudaError_t attr_bf16 = cudaFuncSetAttribute(
        gqa_attention_prefill_bf16_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize,
        kGqaPrefillSmemBytes);
    CUDA_CHECK(attr_bf16);
    static const cudaError_t attr_i8 = cudaFuncSetAttribute(
        gqa_attention_prefill_i8_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize,
...
void gqa_attention_prompt_launch(const Tensor& q, const Tensor& k, const Tensor& v,
    ...
    if (kv.dtype == DType::I8) {
        ...
        const std::int64_t fill_units = static_cast<std::int64_t>(tokens) *
                                        kGqaPrefillKVHeads * kGqaKvQuantGroups;

// qus::kernels - linear Q4 launcher: GEMV/GEMM grid/block/stream setup.
#include "kernels/launcher/linear.h"

#include "kernels/kernel/linear_q4.cuh"
...
int grid_for(std::int64_t n) {
    ...
    if (grid > std::numeric_limits<int>::max()) {
        throw std::overflow_error("linear: Q4 launch grid exceeds CUDA limit");
    }
...
void linear_q4_gemv_launch(const Tensor& x, const Weight& weight, Tensor& out,
                           cudaStream_t stream) {
    const std::int32_t n        = out.ne[0];
    const std::int32_t k        = x.ne[0];
    const std::int32_t padded_k = weight.padded_shape[1];
    linear_q4_gemv_kernel<<<grid_for(n), kBlock, 0, stream>>>(
        static_cast<const __nv_bfloat16*>(x.data), payload_ptr(weight),
        static_cast<__nv_bfloat16*>(out.data), n, k, padded_k);
    CUDA_CHECK(cudaGetLastError());
}

void linear_q4_gemm_launch(const Tensor& x, const Weight& weight, Tensor& out,
                           cudaStream_t stream) {
    const std::int32_t n        = out.ne[0];
    const std::int32_t k        = x.ne[0];
    ...
    linear_q4_gemm_kernel<<<grid_for(elements), kBlock, 0, stream>>>(
        static_cast<const __nv_bfloat16*>(x.data), payload_ptr(weight),

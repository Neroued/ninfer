...
void launch_grouped_cfg(const Tensor& x, RowsplitGroupedJob job0, RowsplitGroupedJob job1,
    ...
    if (full_tiles) {
        linear_rowsplit_grouped_input_gemm_mma_kernel<Cfg, true><<<grid, Cfg::THREADS, 0, stream>>>(
            static_cast<const __nv_bfloat16*>(x.data), job0, job1, job2, job3, x.ne[0], t, x.ne[0]);
    } else {
        linear_rowsplit_grouped_input_gemm_mma_kernel<Cfg, false>
...
void linear_rowsplit_attn_input_grouped_mma_launch(const Tensor& x, const Weight& q_weight,
                                                   ...
                                                   cudaStream_t stream) {
    launch_grouped(x, make_job(q_weight, q, q.ne[0], 0, false),
                   make_job(gate_weight, gate, gate.ne[0], 0, true),
                   make_job(k_weight, k, k.ne[0], 0, false),
                   make_job(v_weight, v, v.ne[0], 0, true), stream);
}

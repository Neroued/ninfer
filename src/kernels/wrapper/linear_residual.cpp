...
void require_q5(const Weight& w) {
    if (w.qtype != QType::Q5G64_F16S || w.layout != QuantLayout::RowSplit ||
        w.q5090_scale_dtype != ScaleDType::FP16 || w.group_size != 64 || w.group != 64 ||
...
void linear_residual_add(const Tensor& x, const Weight& w, Tensor& residual_out, WorkspaceArena& ws,
                         cudaStream_t stream) {
    require_q5(w);

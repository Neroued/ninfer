...
media::internal::Image resize_bicubic(const media::internal::Image& input, Size size) {
...
float normalized(const media::internal::Image& image, int y, int x, int channel) {
...
void append_patch(const std::vector<const media::internal::Image*>& frames, int grid_y, int grid_x,
                  std::vector<float>& out) {
    for (int channel = 0; channel < 3; ++channel) {
        for (int temporal = 0; temporal < kTemporal; ++temporal) {
            const media::internal::Image& frame = *frames[static_cast<std::size_t>(temporal)];
...
Prepared prepare_image(const text::ChatPart& part, const ProcessorOptions& options,
                       const media::internal::Policy& policy, PreprocessStats& stats) {
    media::internal::Image image = media::internal::decode_image(part.source, policy);
    ...
    const std::vector<const media::internal::Image*> frames{&image, &image};
...
Prepared prepare_video(const text::ChatPart& part, const ProcessorOptions& options,
                       const media::internal::Policy& policy, PreprocessStats& stats) {
    media::internal::Video video = media::internal::decode_video(
        part.source, policy, options.video_fps, options.video_min_frames, options.video_max_frames);
    const Size size =
    ...
    add_budget(stats, out.item);
    enforce_budget(stats, options);
    for (media::internal::Image& frame : video.frames) { frame = resize_bicubic(frame, size); }
    ...
    for (int t = 0; t < gt; ++t) {
        const std::vector<const media::internal::Image*> frames{
...
std::vector<const text::ChatPart*> media_parts(const std::vector<text::ChatMessage>& messages) {
    std::vector<const text::ChatPart*> out;
    for (const text::ChatMessage& message : messages) {
        ...
        for (const text::ChatPart& part : message.parts) {
            if (part.kind != text::ChatPartKind::Text) { out.push_back(&part); }
        }
    }
    return out;
}
...
void validate_special_token(const text::QwenTokenizer& tokenizer, std::string_view text,
                            int expected) {
    const std::vector<int> ids = tokenizer.encode(text);
    if (ids.size() != 1 || ids.front() != expected) {
        throw std::invalid_argument("q5090 tokenizer vision token IDs do not match model contract");
    }
}
...
Processor::Processor(const text::QwenTokenizer& tokenizer, ProcessorOptions options)
    ...
    if (options_.max_media_items == 0 || options_.max_prompt_tokens == 0 ||
        ...
        !(options_.max_video_duration_seconds > 0.0) || options_.fetch_connect_timeout_ms <= 0 ||
        options_.fetch_timeout_ms <= 0 || options_.fetch_max_redirects < 0) {
...
ProcessedInput Processor::process(const std::vector<text::ChatMessage>& messages,
    ...
    const media::internal::Policy policy{
        ...
        .connect_timeout_ms         = options_.fetch_connect_timeout_ms,
        .timeout_ms                 = options_.fetch_timeout_ms,
        .max_redirects              = options_.fetch_max_redirects,
        .allow_remote               = options_.allow_remote,
        .allow_private_network      = options_.allow_private_network,
        .media_root                 = options_.media_root,
    };

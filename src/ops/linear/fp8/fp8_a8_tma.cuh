...
// The work distributor hands CTAs to SMs in linear order with blockIdx.x fastest, so the stock
// grid - x over weight-row tiles, y over token tiles - puts a different weight tile in every CTA
// that runs at the same time, and the whole weight matrix is re-read once per token tile. Walking
// the token index fastest instead makes the CTAs that share a weight tile run together, and the
// matrix is read once. Every other route here already makes that choice: the cp.async schedules
// this one falls back to are registered with Fp8MmaRaster::TokenFast, and nvfp4_w4a4_tma.cuh
// carries the same arithmetic. A shared home for it is a follow-up; putting one here would edit
// NVFP4 files this change has no other reason to touch.
__device__ __forceinline__ void fp8_tma_raster_blocks(int& block_x, int& block_y) {
...
__global__ __launch_bounds__(Schedule::kThreads, Schedule::kMinBlocksPerSm) void fp8_a8_tma_kernel(
    ...
    int block_x               = 0;
    int block_y               = 0;
    fp8_tma_raster_blocks(block_x, block_y);
    const int token_begin     = block_y * Schedule::kBlockM;
    const int row_begin       = block_x * kBranchRows;

...
__global__ void linear_rowsplit_gemv_mlp_gate_up_34816_q4_kernel(
    ...
    for (int tile = 0; tile < kGroups; tile += kGroupsPerWarpTile) {
        const auto* global_vecs = reinterpret_cast<const uint4*>(code_row + tile * kBytesPerGroup);
...
void linear_rowsplit_gemv_mlp_gate_up_34816_q4_launch(const Tensor& x, const Weight& w, Tensor& out,

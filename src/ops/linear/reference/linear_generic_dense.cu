// ninfer::ops::detail - generic dense linear GEMV/GEMM launchers.
#include "ops/linear/reference/linear_generic.h"

#include "ops/common/math.h"
#include "ops/linear/reference/linear_generic_dense.cuh"
...
dim3 tiled_gemm_grid_for(std::int32_t n, std::int32_t t) {
    const std::int64_t grid_m =
        div_up(static_cast<std::int64_t>(n), static_cast<std::int64_t>(kDenseGemmBlockRows));
    const std::int64_t grid_n =
        div_up(static_cast<std::int64_t>(t), static_cast<std::int64_t>(kDenseGemmBlockCols));

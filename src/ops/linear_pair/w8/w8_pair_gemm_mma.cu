...
void launch_variant(const Tensor& x, const Weight& first_weight, const Weight& second_weight,
    ...
    w8_pair_gemm_mma_kernel<Variant>
        <<<grid, 256, 0, stream>>>(xp, first_codes, first_scales, second_codes, second_scales,
                                   first_output, second_output, m, k, n, padded_k);
}
...
void w8_pair_gemm_mma_launch(W8KernelVariant variant, const Tensor& x, const Weight& first_weight,
    ...
    case W8KernelVariant::Full:
        launch_variant<W8KernelVariant::Full>(x, first_weight, second_weight, first_out, second_out,
                                              stream);
        break;
    case W8KernelVariant::Predicated:
        launch_variant<W8KernelVariant::Predicated>(x, first_weight, second_weight, first_out,
                                                    second_out, stream);
        break;

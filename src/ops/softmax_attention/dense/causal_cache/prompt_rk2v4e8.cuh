...
// rk2v4-e8 causal prompt kernel for the registered head geometries. Q and cached K share the
// fixed register-only D256 rotation; K is persisted as E8-root cylinder codes at quarter width
// and expands back to the int8 codes the m16n8k32.s8 QK path consumes. V is persisted as packed
// int4 at half width in a per-64-group rotated basis, is unpacked and dequantized with packed
// FP16 arithmetic while producer warps execute QK, and therefore leaves PV in rotated-V
// coordinates - rk2v4e8_inverse_rotate_output_kernel undoes that after this kernel returns.
// Sixteen warps split each 16-row FP16 PV output across four 64-dimension slices.
...
inline constexpr int kCausalPromptRk2v4E8Warps      = 16;
inline constexpr int kCausalPromptRk2v4E8Threads    = kCausalPromptRk2v4E8Warps * 32;
inline constexpr int kCausalPromptRk2v4E8Br         = 64;
inline constexpr int kCausalPromptRk2v4E8Bc         = 64;
inline constexpr int kCausalPromptRk2v4E8Groups     = kCausalPromptHeadDim / kKVCacheInt8Group;
inline constexpr int kCausalPromptRk2v4E8DB16       = kCausalPromptHeadDim / 2;
inline constexpr int kCausalPromptRk2v4E8RowTiles   = kCausalPromptRk2v4E8Br / 16;
inline constexpr int kCausalPromptRk2v4E8DConsumers = kCausalPromptRk2v4E8Warps / kCausalPromptRk2v4E8RowTiles;

inline constexpr int kCausalPromptRk2v4E8QBytes = kCausalPromptRk2v4E8Br * kCausalPromptHeadDim;
inline constexpr int kCausalPromptRk2v4E8QScaleBytes =
    kCausalPromptRk2v4E8Br * kCausalPromptRk2v4E8Groups * static_cast<int>(sizeof(float));
inline constexpr int kCausalPromptRk2v4E8KBytes = kCausalPromptRk2v4E8Bc * kCausalPromptHeadDim;
inline constexpr int kCausalPromptRk2v4E8VBytes = kCausalPromptRk2v4E8Bc * kCausalPromptHeadDim;
inline constexpr int kCausalPromptRk2v4E8VStageBytes =
    kCausalPromptRk2v4E8Bc * kCausalPromptHeadDim * static_cast<int>(sizeof(__half));
inline constexpr int kCausalPromptRk2v4E8PBytes =
    kCausalPromptRk2v4E8Br * kCausalPromptRk2v4E8Bc * static_cast<int>(sizeof(__half));
inline constexpr int kCausalPromptRk2v4E8ScaleBytes =
    2 * kCausalPromptRk2v4E8Bc * kCausalPromptRk2v4E8Groups * static_cast<int>(sizeof(__half));
inline constexpr int kCausalPromptRk2v4E8StatsBytes =
    2 * kCausalPromptRk2v4E8Br * static_cast<int>(sizeof(float));
inline constexpr int kCausalPromptRk2v4E8SmemBytes =
    kCausalPromptRk2v4E8QBytes + kCausalPromptRk2v4E8QScaleBytes + kCausalPromptRk2v4E8KBytes +
    kCausalPromptRk2v4E8VBytes + kCausalPromptRk2v4E8VStageBytes + kCausalPromptRk2v4E8PBytes +
    kCausalPromptRk2v4E8ScaleBytes + kCausalPromptRk2v4E8StatsBytes;

static_assert(kCausalPromptRk2v4E8Groups == 4);
static_assert(kCausalPromptRk2v4E8DConsumers == 4);
static_assert(kCausalPromptRk2v4E8SmemBytes == 92672);

__device__ __forceinline__ int4 causal_prompt_rk2v4e8_dequant_f16x8(const std::int8_t* codes8,
                                                               __half scale) {
...
__global__ __maxnreg__(120) void causal_attention_prompt_rk2v4e8_kernel(
    ...
    constexpr int Br            = kCausalPromptRk2v4E8Br;
    constexpr int Bc            = kCausalPromptRk2v4E8Bc;
    constexpr int DB16          = kCausalPromptRk2v4E8DB16;
    constexpr int Groups        = kCausalPromptRk2v4E8Groups;
    constexpr int GroupKc       = kKVCacheInt8Group / 32;
    constexpr int QKNt          = Bc / 8;
    constexpr int PVNtPerWarp   = D / (kCausalPromptRk2v4E8DConsumers * 8);
    constexpr int PVKs          = Bc / 16;
    constexpr int ProducerWarps = kCausalPromptRk2v4E8RowTiles;
    constexpr int VWorkerWarps  = kCausalPromptRk2v4E8Warps - ProducerWarps;
    ...
    float* q_scale    = reinterpret_cast<float*>(q_i8 + kCausalPromptRk2v4E8QBytes);
    std::int8_t* k_i8 = reinterpret_cast<std::int8_t*>(reinterpret_cast<unsigned char*>(q_scale) +
                                                       kCausalPromptRk2v4E8QScaleBytes);
    std::int8_t* v_i8 = k_i8 + kCausalPromptRk2v4E8KBytes;
    __half* v_f16     = reinterpret_cast<__half*>(v_i8 + kCausalPromptRk2v4E8VBytes);
    __half* p_s       = reinterpret_cast<__half*>(reinterpret_cast<unsigned char*>(v_f16) +
                                                  kCausalPromptRk2v4E8VStageBytes);
    __half* k_scale_s =
        reinterpret_cast<__half*>(reinterpret_cast<unsigned char*>(p_s) + kCausalPromptRk2v4E8PBytes);
    ...
    for (int row = warp; row < Br; row += kCausalPromptRk2v4E8Warps) {
        float q_values[8];
#pragma unroll
        for (int r = 0; r < 8; ++r) {
...
#pragma unroll
    ...
    auto issue_kv_tile = [&](int tile_k0) {
        ...
        for (int key_l = tid; key_l < Bc; key_l += kCausalPromptRk2v4E8Threads) {
            const int key = tile_k0 + key_l;
            __half* kd    = &k_scale_s[key_l * Groups];
            __half* vd    = &v_scale_s[key_l * Groups];
            if (key <= max_query_abs) {
                const std::int64_t off =
                    rk2v4e8_scale_index<Geometry>(physical_page, kv_head, 0, key_l);
...
#pragma unroll 1
        for (int chunk = tid; chunk < Bc * (D / 16); chunk += kCausalPromptRk2v4E8Threads) {
            ...
            if (key <= max_query_abs) {
                ...
                rk2v4e8_decode_k_16d(
                    &cache_k[rk2v4e8_k_code_index<Geometry>(physical_page, kv_head, d, key_l)],
                    decoded);
                store_vec(kd, *reinterpret_cast<const int4*>(decoded));
                rk2v4e8_unpack_v_16d(
                    &cache_v[rk2v4e8_v_code_index<Geometry>(physical_page, kv_head, d, key_l)],
                    decoded);
                store_vec(vd, *reinterpret_cast<const int4*>(decoded));
            } else {
...
#pragma unroll 1
            for (int chunk = worker_tid; chunk < Bc * (D / 8); chunk += WorkerThreads) {
                ...
                if (key <= max_query_abs) {
                    ...
                    store_vec(dst, causal_prompt_rk2v4e8_dequant_f16x8(&v_i8[key_l * D + d], vs));
                } else {
        ...
        const int row_tile = warp % kCausalPromptRk2v4E8RowTiles;
        const int d_slice  = warp / kCausalPromptRk2v4E8RowTiles;

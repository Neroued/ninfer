...
inline DiffStats compute_diff(const double* a, const double* b, long long n,
                              const Tolerance& tol) {
    DiffStats s;
    s.n = n;
...
inline bool diff_passes(const DiffStats& s, const Tolerance& tol) {
    ...
    return frac <= tol.tail_frac
        && s.worst_violation_ratio <= tol.worst_ratio_max
        && s.rel_l2 <= tol.rel_l2_tol;
}

inline void print_diff(const char* label, const DiffStats& s, const Tolerance& tol) {
    std::printf("    %-28s max_abs=%.3e max_rel=%.3e mean_abs=%.3e rel_l2=%.3e",
                label, s.max_abs, s.max_rel, s.mean_abs, s.rel_l2);
    ...
    if (s.n_violating > 0) {
        ...
        if (diff_passes(s, tol)) {
            std::printf(" [tail-allowed: frac<=%.0e worst<=%.0fx rel_l2<=%.0e]",
                        tol.tail_frac, tol.worst_ratio_max, tol.rel_l2_tol);
        }
    }

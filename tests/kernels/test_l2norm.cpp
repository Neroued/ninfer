...
static int one_shape(const char* tag, std::int32_t d0, std::int32_t d1, std::int32_t d2,
    ...
    kernels::l2norm(tx, 1e-6f, tout, nullptr);
...
static int validation_checks() {
    ...
    try {
        ...
        empty_x.ne[0] = 0;
        empty_out.ne[0] = 0;
        kernels::l2norm(empty_x, 1e-6f, empty_out, nullptr);
    ...
    try {
        ...
        kernels::l2norm(bad_empty_x, 1e-6f, bad_empty_out, nullptr);
    ...
    try {
        ...
        kernels::l2norm(huge_x, 1e-6f, huge_out, nullptr);
    ...
    try {
        ...
        kernels::l2norm(bad_dtype, 1e-6f, out, nullptr);
    ...
    try {
        ...
        kernels::l2norm(x, 1e-6f, bad_shape, nullptr);
    ...
    try {
        ...
        bad_stride.nb[0] = 4;
        kernels::l2norm(x, 1e-6f, bad_stride, nullptr);
        std::cerr << "validation contiguous: expected invalid_argument\n";
        ++f;
    } catch (const std::invalid_argument&) {}

    try {
        kernels::l2norm(x, 0.0f, out, nullptr);
        std::cerr << "validation eps zero: expected invalid_argument\n";
        ++f;
    } catch (const std::invalid_argument&) {}

    try {
        kernels::l2norm(x, std::numeric_limits<float>::infinity(), out, nullptr);
        std::cerr << "validation eps finite: expected invalid_argument\n";
        ++f;
    } catch (const std::invalid_argument&) {}

    try {
        kernels::l2norm(x, 1e-6f, out, nullptr);

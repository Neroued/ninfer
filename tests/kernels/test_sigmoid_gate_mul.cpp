// Correctness + coverage for sigmoid_gate_mul, against the frozen op-test standard
...
static int one_shape(const char* tag, std::int32_t d0, std::int32_t d1, std::uint32_t seed,
    ...
    kernels::sigmoid_gate_mul(tgate, tx, nullptr);
...
static int unaligned_data_case() {
    ...
    return verify("sigmoid_gate_mul unaligned data", from_device_bf16(packed, n), ref,
...
static int validation_checks() {
    ...
    try {
        ...
        empty_gate.ne[0] = 0;
        empty_x.ne[0]    = 0;
        kernels::sigmoid_gate_mul(empty_gate, empty_x, nullptr);
    ...
    try {
        ...
        kernels::sigmoid_gate_mul(bad_dtype, x, nullptr);
    ...
    try {
        ...
        bad_shape.ne[2]  = 2;
        kernels::sigmoid_gate_mul(bad_shape, x, nullptr);
    ...
    try {
        Tensor bad_stride = x;
        bad_stride.nb[0]  = 4;
        kernels::sigmoid_gate_mul(gate, bad_stride, nullptr);
        std::cerr << "validation contiguous: expected invalid_argument\n";
        ++f;
    } catch (const std::invalid_argument&) {}

    try {
        kernels::sigmoid_gate_mul(gate, x, nullptr);
...
int main() {
    ...
    for (std::uint32_t seed : {1u, 7u, 99u}) {
        f += one_shape("sigmoid_gate_mul [6144,1]", 6144, 1, seed, -8.f, 8.f);
        f += one_shape("sigmoid_gate_mul [6144,4096]", 6144, 4096, seed, -8.f, 8.f);
        f += one_shape_1d("sigmoid_gate_mul [255]", 255, seed, -8.f, 8.f);
    }
    // Stress: large magnitudes expose range-limited sigmoid approximations.
    f += one_shape_1d("sigmoid_gate_mul stress [-40,40]", 65536, 4242u, -40.f, 40.f);
    f += unaligned_data_case();

    std::cout << (f ? "FAIL" : "OK") << " sigmoid_gate_mul correctness\n";
    return f ? 1 : 0;
}

...
int run_shape(std::int32_t n, std::int32_t k, std::int32_t first_a8, std::uint32_t seed) {
    std::vector<Invocation> invocations{
        ...
        // has to be checked where the store drops rows. 4001 and 4160 leave the route a partial
        // trailing tile, and 4001 is not a whole cp.async token tile - a width the route reaches
        // only since that condition was removed. 5120x6144 is bounded at 4096 and declines 4160.

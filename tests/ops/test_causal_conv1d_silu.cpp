...
// standard (docs/op-development.md): fp64 golden from bf16-rounded
...
static int prefill_state_carry_equivalence(std::uint32_t seed, std::int32_t C, std::int32_t T,
    ...
    ops::causal_conv1d_silu(x0, tw_split, ts_split, out0, nullptr);
    const std::int32_t tail = T - split;
    Tensor x1               = tx_split.slice(1, split, tail);
    Tensor out1             = tout_split.slice(1, split, tail);
    ops::causal_conv1d_silu(x1, tw_split, ts_split, out1, nullptr);
    cudaDeviceSynchronize();
...
static int prefill_from_slot_oracle(std::uint32_t seed, std::int32_t C, std::int32_t T,
    ...
    ops::causal_conv1d_silu(fx_t, fw_t, fin, fout_state, fout_t, nullptr);
    cudaDeviceSynchronize();
...
static int validation_checks() {
    ...
    f += expect_invalid("causal_conv1d validation x dtype", [&] {
        ...
        ops::causal_conv1d_silu(bad, weight, state, out, nullptr);
    });
    f += expect_invalid("causal_conv1d validation weight dtype", [&] {
        Tensor bad = weight;
        bad.dtype  = DType::FP32;
        ops::causal_conv1d_silu(x, bad, state, out, nullptr);
    });
    f += expect_invalid("causal_conv1d validation state dtype", [&] {
        Tensor bad = state;
        bad.dtype  = DType::FP32;
        ops::causal_conv1d_silu(x, weight, bad, out, nullptr);
    });
    f += expect_invalid("causal_conv1d validation out dtype", [&] {
        Tensor bad = out;
        bad.dtype  = DType::FP32;
        ops::causal_conv1d_silu(x, weight, state, bad, nullptr);
    });
    ...
    f += expect_invalid("causal_conv1d validation zero C", [&] {
        ...
        ops::causal_conv1d_silu(zx, zw, zs, zo, nullptr);
    });
    f += expect_overflow("causal_conv1d validation overflow", [&] {
        ...
        hs.ne[2] = 1;
        hs.ne[3] = 1;
        ops::causal_conv1d_silu(hx, hw, hs, ho, nullptr);
    });
    ...
    try {
        ...
        zx.ne[1]  = 0;
        zo.ne[1]  = 0;
        ops::causal_conv1d_silu(zx, weight, state, zo, nullptr);

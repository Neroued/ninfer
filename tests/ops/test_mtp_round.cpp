...
int accept_all_case() {
    ...
    failures += expect_eq("accept all stats", from_device_i64(d_stats, kStepStatsCounters),
                          {3, 3, 1, 0, 1, 1, 1, 0, 0});
    return failures;
}
...
int shifted_case() {
    ...
    ops::mtp_prepare_shifted_ids(verify, token, accepted, shifted, nullptr);
    cudaDeviceSynchronize();
...
int fallback_count_case() {
    ...
    cudaDeviceSynchronize();
    return expect_eq("fallback count", from_device_i64(d_stats, kStepStatsCounters),
...
int main() {
    static_assert(kStepStatsCounters == 9);
    if (cuda_unavailable()) {

...
S expected_schedule(const Q5Problem& problem) {
    ...
    if (problem.rows == 1152 && problem.k == 1152 && problem.padded_k == 1152) {
        ...
        if (problem.cols <= 636) { return S::MmaR64C64; }
        if (problem.cols <= 700) { return S::MmaR64C128; }
        if (problem.cols <= 704) { return S::MmaR64C64; }
        if (problem.cols <= 828) { return S::MmaR64C128; }
        if (problem.cols <= 832) { return S::MmaR64C64; }
        if (problem.cols <= 896) { return S::MmaR64C128; }
        if (problem.cols <= 960) { return S::MmaR64C64; }
        if (problem.cols <= 1024) { return S::MmaR64C128; }
        if (problem.cols <= 1088) { return S::MmaR64C64; }
        return S::MmaR64C128;
    }
...
void route_boundaries() {
    constexpr std::array<Q5Problem, 45> cases{{
        ...
        {5120, 17408, 17408, 24},      {1152, 1152, 1152, 4},    {1152, 1152, 1152, 76},
        {1152, 1152, 1152, 80},        {1152, 1152, 1152, 636},  {1152, 1152, 1152, 640},
        {1152, 1152, 1152, 700},       {1152, 1152, 1152, 704},  {1152, 1152, 1152, 708},
        {1152, 1152, 1152, 828},       {1152, 1152, 1152, 832},  {1152, 1152, 1152, 836},

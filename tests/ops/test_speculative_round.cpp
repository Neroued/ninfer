...
constexpr int kStatsCountersForK5 = 9;
...
int prepare_verify_case() {
    ...
    ops::speculative_prepare_verify_inputs(token, drafts, length, verify, positions, nullptr);
...
int accept_partial_case() {
    ...
    ops::speculative_accept_greedy_drafts(targets, logits, drafts, length, token, sampled, num,
                                          accepted, stats, 16, config_ptr(d_cfg),
    ...
    failures += expect_eq("accept partial stats", from_device_i64(d_stats, kStatsCountersForK5),
...
int accept_all_reject_case() {
    ...
    failures += expect_eq("accept all reject stats", from_device_i64(d_stats, kStatsCountersForK5),
                          {5, 0, 1, 0, 0, 0, 0, 0, 0});
    return failures;
}
...
int accept_all_case() {
    ...
    failures += expect_eq("accept all stats", from_device_i64(d_stats, kStatsCountersForK5),
                          {3, 3, 1, 0, 1, 1, 1, 0, 0});
    return failures;
}

int k15_common_contract_case() {
    constexpr int k     = 15;
    constexpr int vocab = 32;
    std::vector<std::int32_t> targets(k + 1);
    std::vector<std::int32_t> drafts(k);
    for (int i = 0; i <= k; ++i) {
        targets[static_cast<std::size_t>(i)] = i;
    ...
    ops::speculative_accept_greedy_drafts(target_tensor, logits, draft_tensor, length, token,
                                          sampled, num, accepted, stats, vocab, config_ptr(d_cfg),
                                          test_sampling_workspace(), nullptr);
    cudaDeviceSynchronize();

    std::vector<std::int64_t> expected_stats(4 + k, 1);
    expected_stats[0] = k;
    expected_stats[1] = k;
    expected_stats[2] = 1;
    expected_stats[3] = 0;
    int failures      = 0;
    failures += expect_eq("K=15 accepted", from_device_i32(d_accepted, 1), {k});
    failures += expect_eq("K=15 produced", from_device_i32(d_num, 1), {k + 1});
    failures += expect_eq("K=15 length", from_device_i32(d_length, 1), {100 + k + 1});
    failures += expect_eq("K=15 stats", from_device_i64(d_stats, 4 + k), expected_stats);
    return failures;
}
...
int remap_case() {
    ...
    const std::vector<std::int32_t> source{0, 100003, count - 1};
    auto d_map       = to_device_i32(id_map);
    auto d_proposals = to_device_i32(source);
    Tensor proposals(d_proposals.p, DType::I32, {static_cast<int>(source.size())});
    ops::proposal_remap_token_ids(proposals, static_cast<const std::int32_t*>(d_map.p), count,
                                  nullptr);
    cudaDeviceSynchronize();
    return expect_eq("remap proposal tokens", from_device_i32(d_proposals, source.size()),
                     {count - 1, count - 1 - source[1], 0});
}

int increment_case() {
    auto d_value = to_device_i32({23});
    Tensor value(d_value.p, DType::I32, {1});
    ops::increment_i32_scalar(value, nullptr);
    cudaDeviceSynchronize();
...
int fallback_count_case() {
    auto d_stats = to_device_i64({5, 7, 3, 4, 100, 200, 300, 400, 500});
    Tensor stats(d_stats.p, DType::I64, {kStatsCountersForK5});
    Tensor fallback_steps = stats.slice(0, 3, 1);
    ops::increment_i64_scalar(fallback_steps, nullptr);
    cudaDeviceSynchronize();
    return expect_eq("fallback count", from_device_i64(d_stats, kStatsCountersForK5),
...
int reject_sampling_distribution_case() {
    ...
    for (int r = 0; r < N; ++r) {
        ...
        ops::speculative_accept_greedy_drafts(targets, logits, drafts, length, token, sampled, num,
                                              accepted, stats, vocab, config_ptr(d_cfg),
                                              test_sampling_workspace(), nullptr);
...
std::vector<int> run_real_shape_speculative_sequence(const std::vector<float>& logits_h,
...
int reject_sampling_real_shape_reproducible_case() {
    ...
    const int rounds   = 64;
    std::vector<int> a = run_real_shape_speculative_sequence(logits_h, drafts, cfg, rounds);
    std::vector<int> b = run_real_shape_speculative_sequence(logits_h, drafts, cfg, rounds);
    if (a != b) {
    ...
    cfg2.seed                = 9002u;
    std::vector<int> c       = run_real_shape_speculative_sequence(logits_h, drafts, cfg2, rounds);
    if (a == c) {
...
int validation_case() {
    ...
    std::cerr << "speculative_round validation: expected invalid_argument\n";
    return 1;
}

// Runs a single speculative accept round on dense [vocab, k+1] logits with a zeroed
...
// accepted}.
std::vector<int> run_one_speculative_round(const std::vector<float>& logits_h, int vocab, int k,
                                           const std::vector<int>& drafts_h,
                                           ops::SamplingConfig cfg, int length,
                                           int token_domain                        = -1,
...
int exact_k_matrix_case() {
    ...
    for (int k = 1; k <= 5; ++k) {
        ...
        failures += expect_eq("speculative exact stochastic K=" + std::to_string(k),
                              run_one_speculative_round(logits, physical_rows, k, drafts,
                                                        stochastic, 100, token_domain),
                              stochastic_expected);
        ...
        failures += expect_eq("speculative exact greedy K=" + std::to_string(k),
                              run_one_speculative_round(logits, physical_rows, k, greedy_drafts,
                                                        greedy, 100, token_domain, targets, false),
                              greedy_expected);
    }
    if (failures == 0) {
...
int physical_stride_and_token_domain_case() {
    ...
    int failures = 0;
    failures += expect_eq(
        "speculative physical stride + token domain bonus",
        run_one_speculative_round(logits, physical_rows, k, {first}, cfg, 10, token_domain),
        {first, bonus, 2, 1});
    failures += expect_eq(
        "speculative physical stride + token domain rejection",
        run_one_speculative_round(logits, physical_rows, k, {7919}, cfg, 10, token_domain),
        {first, 0, 1, 0});
    if (failures == 0) {
        std::cout << "    speculative physical stride + token domain rejection/bonus ok\n";
    }
    return failures;
}
...
int overlay_frequency_repeat_case() {
    ...
    for (std::uint64_t seed : {1ull, 7ull, 4242ull, 99991ull}) {
        for (int L : {0, 5, 1000, 65536}) {
            cfg.seed             = seed;
            std::vector<int> got = run_one_speculative_round(logits, vocab, k, drafts, cfg, L);
...
int overlay_presence_case() {
    ...
    cfg.seed = 13;
    failures += expect_eq(
        "overlay presence with null committed counts",
        run_one_speculative_round(logits, vocab, k, drafts, cfg, 9, -1, {}, false), expected);
...
// kernels. Exercises the overlay applied inside speculative_sampling_partial_topk_kernel.
int overlay_frequency_repeat_real_shape_case() {
    ...
    for (std::uint64_t seed : {3ull, 88ull, 123457ull}) {
        for (int L : {0, 1000, 250000}) {
            cfg.seed = seed;
            std::vector<int> got =
                run_one_speculative_round(logits, vocab, k, drafts, cfg, L, token_domain);
...
int main() {
    static_assert(kStatsCountersForK5 == 9);
    if (cuda_unavailable()) {
    ...
    failures += k15_common_contract_case();

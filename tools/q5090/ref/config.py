"""The single Python model card for Qwen3.6-27B."""
...
class ModelConfig:
    ...
    def validate_header(self, header: Mapping[str, object]) -> None:
        expected = {
            ...
            "gdn_key_heads": self.gdn_k_heads,
            "gdn_value_heads": self.gdn_v_heads,
            "gdn_key_head_dim": self.gdn_k_dim,
            "gdn_value_head_dim": self.gdn_v_dim,
            "gdn_conv_width": self.conv_width,
            "full_attention_interval": self.full_interval,
        }
        bad = [f"{key}={header.get(key)!r}, expected {value}" for key, value in expected.items()
               if header.get(key) != value]
        if bad:
            raise ValueError("q5090 model card mismatch: " + "; ".join(bad))
